...
function Install-Python {
    ...
    if ($UserInput) {
        if ($Processor -like "64-bit") {
            Invoke-WebRequest -URI "https://www.python.org/ftp/python/3.10.5/python-3.10.5-amd64.exe" -OutFile "python-3.10.5.exe"
        } else {
            Invoke-WebRequest -URI "https://www.python.org/ftp/python/3.10.5/python-3.10.5.exe" -OutFile "python-3.10.5.exe"
        }

        python-3.10.5.exe /quiet
    }
}
