    ...
    if search_input == None:
        ...
        if configs_file["interface"] == "cli":
            ...
            for index, element in enumerate(found_elements):
                ...
                stdout.write(
                    f"Type:{AnsiEscapeCodes.RESET} {ALL_COLOERS[class_type]}{full_course_type}{AnsiEscapeCodes.RESET}{colors}, "
                )
                stdout.write(
                    f"Available Seats: {full_course_available_seats}, Waiting List: {full_course_waiting_list}, "
                )
                stdout.write(
                    "CRN:{} {}{}{}{}".format(
        ...
        if configs_file["interface"] == "cli":
            for i in search_input:
                if i == "section":
                    found_elements = list(
                        filter(
                            lambda x: x["sequenceNumber"] in search_input["section"],
                            found_elements,
                        )
                    )
                elif i == "activity":
                    ...
                    found_elements = list(
                        filter(
                            lambda x: x["courseReferenceNumber"] in search_input["crn"],
                            found_elements,
                        )
                    )
                    ...
                    found_elements = list(
                        filter(
                            ...
                            in search_input["instructor_name"],
                            found_elements,
                        )
                    )
                ...
                #     (filter(lambda x: x["building"] in search_user_input["building"], found_elements))
                    ...
                    found_elements = list(
                        filter(
                            lambda x: x["meetingsFaculty"][0]["meetingTime"]["building"]
                            in search_input["building"],
                            found_elements,
                        )
                    )
                elif i == "status":
                    if search_input["status"] == "Open":
                        found_elements = list(
                            filter(lambda x: x["seatsAvailable"] > 0, found_elements)
                        )
                    else:
                        found_elements = list(
                            filter(
