    ...
    if (configs_file["interface"] == "cli"):
        if (configs_file["browser"] != None):
            ...
            if (registrar_answer == True):
                ...
                while not finished:
                    crns_list = []
                    crns_str = Questions.str_questoin(question="Enter CRN/CRNs you want to registrar (separated by spaces)")
                    for crn in crns_str.strip().split(" "):
                        if (crn.isdigit()):
                            if (10000 <= int(crn) <= 39999):
                                ...
                                print(f"{AnsiEscapeCodes.RED}! Sorry, your reply was invalid:{AnsiEscapeCodes.RESET}",
        ...
        if ("Check `ALL` courses of the department" in search_by):
            ...
            for i in search_by:
                if (i == "Section/Sections"):
                    ...
                    while not finished:
                        sections_list = []
                        sections_str = Questions.str_questoin(question="Enter Section/Sections you want to check each refresh")
                    ...
                    while not correct_answer:
                        ...
                        for index, element in enumerate(courses_names_str.split(" ")):
                            if ((element.isalpha()) and (index % 2 == 1)) or ((element.isdigit()) and (index % 2 == 0)):
                                stdout.write(
                                    f"{AnsiEscapeCodes.RED}! Sorry, your reply was invalid:{AnsiEscapeCodes.RESET}",
                                    f"{AnsiEscapeCodes.BOLD}\"{crn}\"{AnsiEscapeCodes.RESET}",
                                    f"{AnsiEscapeCodes.RED}is not a valid answer, please try again.{AnsiEscapeCodes.RESET}"
                ...
                #     filter_dictionary["course_number"] = times_str.split(" ")
                    ...
                    while not correct_answer:
                        buildings_list = []
                        buildings_str = Questions.str_questoin(question="Enter Building/Buildings you want to check each refresh")
